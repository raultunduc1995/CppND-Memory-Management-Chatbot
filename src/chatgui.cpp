...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event)) {
  ...
  _panelDialog->SendMessageToChatbot(userText);
}
...
ChatBotPanelDialog::ChatBotPanelDialog(wxWindow *parent, wxWindowID id)
  ...
  _chat_logic = std::make_unique<ChatLogic>();
...
void ChatBotPanelDialog::SendMessageToChatbot(const wxString &user_message) {
  auto user_message_string = std::string(user_message.mb_str());
  _chat_logic->SendMessageToChatbot(user_message_string);
}

wxBitmap *ChatBotPanelDialog::GetImageFromChatbot() {
  return _chat_logic->GetImageFromChatbot();
}
...
ChatBotPanelDialogItem::ChatBotPanelDialogItem(wxPanel *parent, wxString text,
  ...
  wxBitmap *bitmap =
      isFromUser == true
          ? nullptr
          : ((ChatBotPanelDialog *)parent)->GetImageFromChatbot();
